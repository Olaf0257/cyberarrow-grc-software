...
class TemplateControlsController extends Controller
{
    ...
    public function create(Request $request, Standard $standard)
    {
        ...
        $idSeparators = $control->idSeparators;
    ...
    public function store(Request $request, Standard $standard)
    {
        ...
        return redirect(route('compliance-template-view-controls', [$standard->id]));
    }
    ...
    public function uploadCsvStore(Request $request, Standard $standard)
    {
        ...
        if ($csvIsEmpty) {
            return back()->with('csv_upload_errors', 'Csv file is empty');
        }

        // show error when Csv file row has not more than one row
        if (count($csvDatas) < 2) {
            return back()->with('csv_upload_errors', true)->withErrors('Csv file is missing body content');
        }
        ...
        if (count($headerDiff) > 0) {
            return back()->with('csv_upload_errors', true)->withErrors('Csv file do not have required headers');
        }
        ...
        if (isset($messages['csv_upload_errors'])) {
            return back()->with('csv_upload_errors', true)->withErrors($messages['csv_upload_errors'][0]);
        }
