...
<div class="row compliance_collapse_tabs">
	<div class="col-xl-12">
		<div class="card">
			<div class="card-body pt-0">
				<a href="{{ route('compliance.projects.export', $project->id) }}" class="btn btn-primary export__risk-btn float-ends">Export</a>
				<ul class="nav nav-tabs nav-bordered">
					<li class="nav-item">
						<a href="#details" data-toggle="tab" aria-expanded="false" class="nav-link active" id="compliance-detail">
							Details
						</a>
					</li>
					<li class="nav-item">
						<a href="#controls" data-toggle="tab" aria-expanded="true" class="nav-link" id="compliance-controls">
							Controls
						</a>
					</li>
				</ul>
				...
				<div class="tab-content">
					<div class="tab-pane active" id="details">
						...
						<p>{{decodeHTMLEntity($project->description)}}</p>
					</div>

					<div class="tab-pane show" id="controls">
						<form action="{{route('compliance-project-controls-update-all', [$project->id])}}" method="POST" id="update-control-assignment-form">
							@csrf
						...
						@if(Auth::guard('admin')->user()->hasAnyRole(['Global Admin', 'Compliance Administrator']))
							<div class="save-button d-flex justify-content-end">
								<button class="btn btn-primary" data-style="expand-right" id="update-control-assignment-bottom-btn">
									<span class="ladda-label">
										Save
									</span>
									<span class="ladda-spinner"></span>
...
@php
	...
	$notImplementedcontrols = $project->controls()->where('applicable', 1)->where('status', 'Not Implemented')->count();
    $rejected = $project->controls()->Where('status', 'Rejected')->count();
    $notImplemented = $notImplementedcontrols + $rejected;
...
@endphp
<div class="row compliance_collapse_tabs1">
	<div class="col-lg-12">
...
$(document).ready(function () {
    ...
    $(document).on('click','#compliance-controls',function(){

        $( ".compliance_collapse_tabs" ).addClass( "project-detail_formrow" );
        $( ".compliance_collapse_tabs1" ).addClass( "compilance-project_controlhead" );
    })
    $(document).on('click','#compliance-detail',function(){

        $( ".compliance_collapse_tabs" ).removeClass( "project-detail_formrow" );
        $( ".compliance_collapse_tabs1" ).removeClass( "compilance-project_controlhead" );
    })
	...
	var basicDataTable = $("#basic-datatable").DataTable({
		...
		ajax: {
			"url": "{{ route('compliance-project-controls', [$project->id]) }}",
			"type": "GET",
		},
    ...
    basicDataTable.on('preXhr.dt', function () {
        tempScrollTop = $(window).scrollTop();
    })

    basicDataTable.on('xhr.dt', function (e, settings, json, xhr) {
        //const pageInfo = basicDataTable.page.info();
        $('html, body').animate({
            scrollTop: json.data.length !== 10 ? 0 : tempScrollTop
        }, 400);
    });
	...
	$(document).on('submit','#update-control-assignment-form',function(e) {
        ...
        var is_validate =  tablevalidatehandle();
		 ...
		 if(is_validate){
            ...
            var updateControlAssignmentBottomBtn = Ladda.create( document.querySelector( '#update-control-assignment-bottom-btn' ) );
			updateControlAssignmentBottomBtn.start();
			...
			$.ajax({
				...
				success: function(response) {
                    ...
                    updateControlAssignmentBottomBtn.stop()
				...
				if( (responsible.val() && approver.val() ) && responsible.val() != approver.val() ){
					...
					if(!responsible.val() || !approver.val() || (responsible.val() == approver.val()) ){
						...
						if( !responsible.val() ){
                            ...
                            $(responsible).siblings('span').children('.selection').after(errorMessage);
						...
						if( !approver.val() ){
                            ...
                            $(approver).siblings('span').children('.selection').after(errorMessage);
